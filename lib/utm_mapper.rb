...
module Mapper
  class Base
    ...
    def link(product)
      begin
        ...
        if response[:count] > 0
          ...
          p shop_item
          linked = check_models(shop_item["model"][0], storage_item_model) || check_titles(shop_item["title"][0], product["title"])
          p "Linked: #{linked}"
    ...
    #TODO: добавити порівняння Назва товару арт. [артикул] з моделлю
